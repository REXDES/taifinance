-- Update handle_new_user function to copy company_limit
...
BEGIN
    ...
    IF invitation_record.id IS NOT NULL THEN
        ...
        -- Inserir na tabela user_roles with company_limit
        INSERT INTO public.user_roles (user_id, role, company_limit)
        VALUES (NEW.id, invitation_record.role, invitation_record.company_limit);
...
-- Allow gerentes to create companies if they have company_limit > 0
CREATE POLICY "Gerentes can create companies within limit"
ON public.companies FOR INSERT
WITH CHECK (
    has_role(auth.uid(), 'gerente') AND 
    (SELECT company_limit FROM public.user_roles WHERE user_id = auth.uid()) > 
    (SELECT COUNT(*) FROM public.companies WHERE created_by = auth.uid())
);

-- Allow gerentes to view companies they created
CREATE POLICY "Gerentes can view companies they created"
ON public.companies FOR SELECT
USING (created_by = auth.uid());

-- Allow gerentes to manage companies they created
CREATE POLICY "Gerentes can manage companies they created"
