...
export default function NectaHomologationPublic() {
  ...
  const save = async () => {
    setSaving(true);
    try { await call(token, { action: 'save', fields: form }); toast.success('Dados salvos'); }
  ...
  const submit = async () => {
    setSaving(true);
    try { await save(); await call(token, { action: 'submit', accept_terms: accepted }); await load(); setStep(6); }
  ...
  return <main className="min-h-screen bg-background px-4 py-8">
    <div className="mx-auto max-w-3xl space-y-5">
      ...
      {step < 4 && <Card><CardHeader><CardTitle>{sections[step].title}</CardTitle></CardHeader><CardContent className="grid gap-4 sm:grid-cols-2">
      ...
      <div className="flex justify-between"><Button variant="outline" disabled={step === 0} onClick={() => setStep((current) => current - 1)}>Voltar</Button>{step < 5 && <Button onClick={async () => { await save(); setStep((current) => current + 1); }}>Salvar e continuar</Button>}</div>
    </div>
  </main>;
}
