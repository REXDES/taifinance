...
export function TagPicker({
  ...
  const { tags, createTag } = useFinanceTags(companyId);
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button
          ...
          variant="outline"
          size={size}
          className={cn('justify-start font-normal min-h-9 h-auto py-1.5 w-full', className)}
        >
          <TagIcon className="w-3.5 h-3.5 mr-2 text-muted-foreground shrink-0" />
          {selected.length === 0 ? (
